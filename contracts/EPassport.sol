...
contract EPassport is Ownable {
    ...
    event passportApproved(
        string passportIdHash,
        address wallet_address,
        address issuer
    );
    event passportBanned(
        string passportIdHash,
        address wallet_address,
        address kyc_center
    );
    ...
    function applyForPassport(
        string memory passportIdHash,
        string memory ipfs_link
    ) public payable {
        //   require (msg.value == _passportFee, "Passport fee is not paid");
        require(
            hashToeid[passportIdHash] == 0,
            "this passportID number already applied"
        );
        passport_eid_count.increment();
        uint current = passport_eid_count.current();
        eids[msg.sender] = current;
        CitizenPassport memory cp = CitizenPassport(
            passportIdHash,
            ipfs_link,
            msg.sender,
            false,
        ...
        passports[current] = cp;
        hashToeid[passportIdHash] = current;
    ...
    function getPassportByIdHash(string memory passportIdHash)
        public
        view
        returns (CitizenPassport memory)
    {
        CitizenPassport memory id_card;
        uint256 eid = hashToeid[passportIdHash];
        id_card = passports[eid];
    ...
    function getPassportByEid(uint256 eid_)
        public
        view
        returns (CitizenPassport memory)
    {
